    ...
    function ajax_save_edit(data) {
        $.ajax({
            type: 'POST',
            url: '/softwayliving/modify_member/',
            data: data,
    ...
    function ajax_unlock_member(t, data) {
        $.ajax({
            type: 'POST',
            url: '/softwayliving/unlock_member/',
            data: data,
            success: function (response) {
                if (response.error != null) {
                    Notiflix.Report.Failure('錯誤', response.error, 'ok');
    ...
    function edit_member(t, user_id, username, phone, birth) {
    ...
    function lock_member(t, user_id) {
        Notiflix.Confirm.Show('管理者權限', '確定要凍結此會員嗎?', '是-凍結會員', '否',
            function () {
                var data = {
                    'user_id': user_id
                }
                //spinner show
                Notiflix.Loading.Hourglass('正在凍結....');
                ajax_lock_member(t, data)

            },
            function () {});
    }

    function unlock_member(t, user_id) {
        Notiflix.Confirm.Show('管理者權限', '確定要解除凍結嗎?', '是-解除', '否',
            function () {
                var data = {
                    'user_id': user_id
                }
                //spinner show
                Notiflix.Loading.Hourglass('正在解除凍結....');
                ajax_unlock_member(t, data)
    ...
    function switch_list(action) {
        if (action == 'ban') {
            document.getElementById('ban_list').style.display = 'inline'
            document.getElementById('normal_list').style.display = 'none'
        } else if (action == 'normal') {
            document.getElementById('ban_list').style.display = 'none'
