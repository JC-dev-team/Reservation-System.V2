...
status=false;}
if(status){send()}});});var user=$.cookie('uu');var pwd=$.cookie('pp');$(document).ready(function(){if(user){$("#email").val(user);$("#password").val(pwd);$("#che").html("<input type=\"checkbox\" onclick=\"uncheck()\" id=\"check1\" checked/>");}});function check(){$("#che").html("<input type=\"checkbox\" onclick=\"uncheck()\" id=\"check1\"/>");document.getElementById("check1").checked=true;$.cookie('uu',$("input:text").val(),{expires:7});$.cookie('pp',$("input:password").val(),{expires:7});}
