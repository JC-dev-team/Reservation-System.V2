;$('#displayName').html('大狂'+'('+'電話訂位'+')');$('#displayPhone').html('0912345678');function reservation_submit(){if('ce3d1adb-fa4e-11e9-9469-0e0140c2e1b2'==null||'ce3d1adb-fa4e-11e9-9469-0e0140c2e1b2'==''){Notiflix.Loading.Hourglass('訂位處理中...');reservation_form_submit()}else if('ce3d1adb-fa4e-11e9-9469-0e0140c2e1b2'!=null&&'ce3d1adb-fa4e-11e9-9469-0e0140c2e1b2'!=''){Notiflix.Loading.Hourglass('訂位處理中...');document.getElementById("reservation_form").action="/softwayliving/insert_bk/";document.getElementById("reservation_form").submit();}
...
$.ajaxSetup({beforeSend:function(xhr,settings){if(!csrfSafeMethod(settings.type)&&!this.crossDomain){xhr.setRequestHeader("X-CSRFToken",csrftoken);}}});function people_number(data,num,calendar){$.ajax({type:'GET',url:'/booking/getCalendar/',data:data,success:function(response){if(response.error!=null){Notiflix.Report.Failure('錯誤',response.error,'ok');}else if(response.alert!=null){Notiflix.Report.Warning('警告',response.alert,'ok');}else if(response.outdated!=null){Notiflix.Report.Warning('警告',response.outdated,'ok');window.location.replace(response.action)}else{result=response.result
...
if(date.getDay()===6){return'六';}},plugins:['interaction','dayGrid','moment'],editable:true,defaultView:'dayGridMonth',defaultDate:(new Date().addDays(3).addMonths(num)).toISOString().slice(0,10),height:'auto',contentHeight:500,header:{left:'prev',center:'title',right:'next'},titleFormat:'YYYY  MM月',showNonCurrentDates:false,selectable:true,events:[{id:'1',title:'午餐',start:new Date(),startRecur:new Date().addDays(3),backgroundColor:'green',textColor:'white',daysOfWeek:[1,2,3,4,5,6,0]},{id:'2',title:'晚餐',start:new Date(),startRecur:new Date().addDays(3),backgroundColor:'green',textColor:'white',daysOfWeek:[1,2,3,4,5,6,0]},],eventRender:function(info){var st=info.event.start
...
liff.init(function(data){initializeApp(data);});$('#adult_select, #children_select').change(function(e){document.getElementById('calendar').innerHTML=''})
