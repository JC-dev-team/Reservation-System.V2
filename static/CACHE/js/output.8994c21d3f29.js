;$('#displayName').html('Yy'+'('+'James'+')');$('#displayPhone').html('0900789789');function reservation_submit(){if(''==null||''==''){Notiflix.Loading.Hourglass('訂位處理中...');reservation_form_submit()}else if(''!=null&&''!=''){Notiflix.Loading.Hourglass('訂位處理中...');document.getElementById("reservation_form").action="/softwayliving/insert_bk/";document.getElementById("reservation_form").submit();}
...
$('#button_submit_test').click(function(e){Notiflix.Confirm.Show('訂位資訊','確定送出訂單','是','否',function(){},function(){});});$('#button_submit').click(function(e){var status=true;$('#store_id').val($('#store_name_select').val());$('#adult').val($('#adult_select').val())
