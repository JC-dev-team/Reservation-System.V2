    ...
    function ajax_delete_admin(t, data) {
        $.ajax({
            type: 'POST',
            url: '/softwayliving/DeleteAdmin/',
            data: data,
            success: function (response) {
                if (response.error != null) {
                    Notiflix.Report.Failure('錯誤', response.error, 'ok');
                    ...
                    Notiflix.Notify.Success('已刪除管理者！！');
    ...
    function save_add() {
        Notiflix.Confirm.Show('管理者權限', '確定要新增嗎?', '是-新增', '否',
            function () {
                ...
                var staff_name = $('#add_staff_name').val();
                var staff_phone = $('#add_staff_phone').val();
                var staff_email = $('#add_email').val();
                var auth = $('#add_auth').val();


                if (staff_name == '' || staff_name == null) {
                    Notiflix.Notify.Failure('請輸入管理者姓名！！')
                    status = false;
                } else if (staff_phone == '' || staff_phone == null) {
                    Notiflix.Notify.Failure('請輸入管理者電話！！')
                    status = false;
                } else if (staff_email == '' || staff_email == null) {
                    Notiflix.Notify.Failure('請輸入管理者信箱！！')
                    status = false;
                } else if (auth == '' || auth == null) {
                    Notiflix.Notify.Failure('請輸入管理者權限！！')
                    status = false;
                }
    ...
    function delete_admin(t) {
        ...
        Notiflix.Confirm.Show('管理者權限', '確定要永久刪除嗎?', '是-刪除', '否',
            function () {
                var data = {
                    'staff_id': delete_staff_id,
                    'email': delete_email
                }
                
                //spinner show
                Notiflix.Loading.Hourglass('正在刪除....');
                ajax_delete_admin(t, data)
